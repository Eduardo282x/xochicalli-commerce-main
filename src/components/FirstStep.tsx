...
const FirstStep = ({ setValue, values }: { setValue: any; values: any }) => {
    ...
    switch (name) {
      case "category":
        const dataSubCategory =
          dataCategorias[value].subCategorys.subcateogory0.nameCategory;
  ...
  const tags = (subCategoryForm: string, categoryy = values.category) => {
    for (const tags in dataCategorias[categoryy]?.subCategorys) {
      if (
        dataCategorias[categoryy]?.subCategorys[tags].nameCategory ===
        subCategoryForm
      ) {
        const data =
          dataCategorias[categoryy]?.subCategorys[tags]?.subCategorys[0]
            ?.value ?? [];
        setArrayTags(
          dataCategorias[categoryy]?.subCategorys[tags]?.subCategorys[0]
            ?.value ?? []
